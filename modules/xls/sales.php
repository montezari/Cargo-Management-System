...
class TExcelRptSales extends TExcel {
  ...
  global $conn, $config, $LABEL;  
	...
	if($rs->RecordCount()>0){
	  $xlstpl = new TExcelTemplate("xls_template/sales.xls");
	  ...
	  */
	  ...
	  header('Content-Disposition: attachment;filename="rekapitulasi_penjualan_detail_'.$dtfile.'.xls"');
